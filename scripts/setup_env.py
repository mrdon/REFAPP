...
if (not os.path.isdir(maven_m2_location)) or (not os.path.isfile(maven_settings_location)):
    ...
    if choice == "yes":
        ...
        setting_wget_cmd = """cd %s;
                              wget https://svn.atlassian.com/svn/private/atlassian/maven2settings/trunk/settings.xml.developer -q -O settings.xml.developer --http-user="%s" --http-password="%s" """ % (maven_m2_location, crowd_user, crowd_passwd)
        ...
        sed_replace_cmd =  """cd %s;
                              sed "s/<username>yourusername<\/username>/<username>%s<\/username>/g" settings.xml.developer | sed "s/<password>yourpassword<\/password>/<password>%s<\/password>/g" > settings.xml""" % (maven_m2_location, crowd_user.replace("/", "\/"), crowd_passwd.replace("/", "\/"))
...
if not os.path.isfile("build_refapp.py"):
    ...
    if choice == "yes":
        script_wget_cmd = """wget https://studio.atlassian.com/svn/REFAPP/trunk/scripts/build_refapp.py --http-user="%s" --http-password="%s" -q -O build_refapp.py """ % (crowd_user, crowd_passwd)
